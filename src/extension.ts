...
class AxamlDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
  ...
  public provideDocumentSymbols(
    ...
    console.log(
      `🔍 [${timestamp}] Document symbol provider called for: ${document.fileName}`
    );
    ...
    const isAxamlFile =
      document.fileName.toLowerCase().endsWith(".axaml") ||
  ...
  public parseAxaml(text: string): AxamlElement[] {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      if (tagMatch) {
        ...
        const attributesMatch = fullTagContent.match(
   ...
   */
  private convertToDocumentSymbols(
    elements: AxamlElement[],
    document: vscode.TextDocument
  ): vscode.DocumentSymbol[] {
    return elements.map((element) =>
      this.createDocumentSymbol(element, document)
    );
  }
  ...
  private createDocumentSymbol(
    ...
    const selectionRange = new vscode.Range(
      ...
      new vscode.Position(
        element.startLine,
        element.startColumn + element.tagName.length + 1
      )
    );
    ...
    symbol.children = element.children.map((child) =>
      this.createDocumentSymbol(child, document)
    );
  ...
  private getSymbolKind(tagName: string): vscode.SymbolKind {
    ...
    if (
      lowerTagName.includes("window") ||
      lowerTagName.includes("usercontrol")
    ) {
    ...
    // Layout containers
    if (
      lowerTagName.includes("grid") ||
      lowerTagName.includes("stackpanel") ||
      lowerTagName.includes("canvas") ||
      lowerTagName.includes("dockpanel") ||
      lowerTagName.includes("wrappanel") ||
      lowerTagName.includes("border")
    ) {
    ...
    if (
      lowerTagName.includes("button") ||
      lowerTagName.includes("checkbox") ||
      lowerTagName.includes("radiobutton") ||
      lowerTagName.includes("slider")
    ) {
    ...
    // Text controls
    if (
      lowerTagName.includes("textblock") ||
      lowerTagName.includes("textbox") ||
    ...
    // List controls
    if (
      lowerTagName.includes("listbox") ||
      lowerTagName.includes("combobox") ||
      lowerTagName.includes("datagrid") ||
      lowerTagName.includes("treeview")
    ) {
    ...
    // Images and media
    if (
      lowerTagName.includes("image") ||
      lowerTagName.includes("mediaElement")
    ) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log(
    `🔤 Active language: ${
      vscode.window.activeTextEditor?.document.languageId || "None"
    }`
  );
  ...
  vscode.window
    .showInformationMessage(
      `🎉 AXAML Extension ACTIVATED at ${new Date().toLocaleTimeString()}!`,
      "Show Status",
      "OK"
    )
    .then((choice) => {
      if (choice === "Show Status") {
        vscode.commands.executeCommand("avalonia-document-outline.status");
      }
  ...
  const testCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.test",
    () => {
      const testInfo = {
      ...
      vscode.window.showInformationMessage(
        `Extension Test: Activated=${
          testInfo.activated
        }, File=${testInfo.activeFile.split("\\").pop()}`
      );
    }
  );
  ...
  const onDidChangeActiveEditor = vscode.window.onDidChangeActiveTextEditor(
    (editor) => {
      if (editor) {
        ...
        console.log(
          `📄 File extension: ${editor.document.fileName.split(".").pop()}`
        );

        if (editor.document.fileName.endsWith(".axaml")) {
          console.log(
            "🎯 AXAML file detected! Document symbols should be available."
          );
  ...
  const onDidChangeTextEditorSelection =
    vscode.window.onDidChangeTextEditorSelection((event) => {
      if (
        event.textEditor &&
        event.textEditor.document.fileName.endsWith(".axaml")
      ) {
  ...
  if (vscode.window.activeTextEditor) {
    ...
    console.log(
      `📄 Extension activated with active editor: ${editor.document.fileName}`
    );
    if (editor.document.fileName.endsWith(".axaml")) {
  ...
  const manualActivateCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.activate",
    () => {
      ...
      if (editor) {
        ...
        vscode.window.showInformationMessage(
          "Avalonia Document Outline manually activated"
        );
      } else {
  ...
  const statusCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.status",
    () => {
      const editor = vscode.window.activeTextEditor;
      const status = {
      ...
      vscode.window
        .showInformationMessage(
          `Extension Status: ${
            status.extensionActive ? "✅ Active" : "❌ Inactive"
  ...
  const reloadCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.forceReload",
    async () => {
      ...
      vscode.languages.registerDocumentSymbolProvider(
        { language: "axaml" },
        newProvider,
        { label: "Avalonia Document Outline (Reloaded)" }
      );
  ...
  const navigateToElementCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.navigateToElement",
  ...
  const showElementInfoCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.showElementInfo",
    () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor || !editor.document.fileName.endsWith(".axaml")) {
        vscode.window.showWarningMessage("No active AXAML file found");
...
${attributes || "  (none)"}`;
        ...
        vscode.window.showInformationMessage(
          "No AXAML element found at cursor position"
        );
      }
    }
  );
  ...
  const toggleLineNumbersCommand = vscode.commands.registerCommand(
    "avalonia-document-outline.toggleLineNumbers",
    async () => {
      showLineNumbers = !showLineNumbers;

      const status = showLineNumbers ? "enabled" : "disabled";
      vscode.window.showInformationMessage(
        `Avalonia Document Outline line numbers ${status}`
      );
      ...
      if (editor && editor.document.fileName.endsWith(".axaml")) {
        ...
        console.log(
          `🔄 Forced outline refresh, symbols: ${
            symbols ? "generated" : "none"
          }`
        );
        ...
        try {
          ...
          await editor.edit((editBuilder) => {
            editBuilder.delete(
              new vscode.Range(position, new vscode.Position(0, 1))
            );
          });
        } catch (error) {
          console.log(
            "⚠️ Could not trigger edit refresh, but symbols should still update"
          );
        }
      }
    }
  ...
  outputChannel.appendLine(
    `AXAML Extension activated at ${activationTimestamp}`
  );
  outputChannel.appendLine(
    `Active file: ${
      vscode.window.activeTextEditor?.document.fileName || "None"
...
async function selectElementInOutline(element: AxamlElement) {
  ...
  try {
    ...
    const tagStart = new vscode.Position(
      element.startLine,
      element.startColumn
    );
    const tagEnd = new vscode.Position(
      element.startLine,
      element.startColumn + element.tagName.length + 1
    );
    ...
    console.log(
      `🎯 Moved cursor to element: ${element.name} (${
        element.tagName
      }) at line ${element.startLine + 1}`
    );
  } catch (error) {
...
function onCursorPositionChanged(editor: vscode.TextEditor) {
  ...
  if (currentDocument !== editor.document) {
    ...
    try {
      ...
      currentDocument = editor.document;
      console.log(
        `🌳 Parsed ${currentAxamlElements.length} elements for selection tracking`
      );
    } catch (error) {
  ...
  if (element) {
    console.log(
      `📍 Cursor at line ${position.line + 1}, found element: ${
        element.name
      } (${element.tagName})`
    );
...
function navigateToElement(editor: vscode.TextEditor, element: AxamlElement) {
  ...
  // Show status message
  vscode.window.setStatusBarMessage(
    `Navigated to: ${element.name} (${element.tagName}) at line ${
      element.startLine + 1
    }`,
    3000
  );
  ...
  console.log(
    `🧭 Navigated to element: ${element.name} (${element.tagName}) at line ${
      element.startLine + 1
    }`
  );
}
